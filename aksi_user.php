...
if($_GET['act'] == 'a'){
	$id		= $_GET['id'];
	$cek = mysqli_query($con, "SELECT * FROM user where id_user = 3 and is_active = '0'");
	if(mysqli_num_rows($cek) > 0) {
		mysqli_query($con, "UPDATE user SET is_active = '1' where id_user= 3");
	}else{
		mysqli_query($con, "UPDATE user SET is_active = '0' where id_user= 3");
	}
